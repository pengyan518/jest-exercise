...
export const fetchContent = async (url, callback) => {
...
function App() {
  ...
  const searchByBreed = () => {
    ...
    fetchContent(`https://dog.ceo/api/breed/${selectedBreed}/images`, cb)
  }
  ...
  useEffect(() => {
    ...
    fetchContent('https://dog.ceo/api/breeds/list/all', cb)
  }, [])
    ...
    <div className="d-flex justify-content-center flex-column text-center">
      ...
      <main role="main">
        <div className="d-flex justify-content-center">
          ...
          <button type="button" className="btn btn-info mx-2" disabled={!selectedBreed} onClick={searchByBreed}>
            Search
          </button>
        </div>
        ...
        <div className="mt-5 d-flex justify-content-center flex-wrap px-5 mx-5">
          {dogImages.length === 0 && !isLoading && <img src={placeholderImg} className="mx-auto d-block mt-4 w-50" alt="" />}
          {isLoading && (
            <div className="d-flex align-items-center">
              <p className="h1 me-2">Loading</p>
              <div className="spinner-border ms-auto text-info fs-3" role="status" aria-hidden="true" />
            </div>
          )}
          {dogImages.length > 0 &&
            !isLoading &&
